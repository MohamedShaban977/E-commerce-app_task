...
class CustomLoadingWidget extends StatelessWidget {
  const CustomLoadingWidget({Key? key, required this.inAsyncCall, required this.child,  this.isCondition =false}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    if(isCondition){
      if (inAsyncCall) {  return  Scaffold(
  ...
  LottieBuilder _buildLoader() {
    return Lottie.asset(JsonAssets.loader,
      width:150,
    );
  }
}
