...
import 'package:e_commerce_app_task/features/auth/login/domain/entities/login_entity.dart';
...
class LoginRepositoryImpl implements LoginRepository {
  ...
  Future<Either<Failure, LoginEntity>> loginUser(LoginRequest request) async {
    try {
      ...
      res.refresh.isNotEmpty? await sl<CacheHelper>().saveData(
              key: Constants.cachedDataLogin,
              value: json.encode(res.access),
            ) : null;
      return res.refresh.isNotEmpty?  Right(res) : left(const ServerFailure(AppStrings.errorOccurred));
